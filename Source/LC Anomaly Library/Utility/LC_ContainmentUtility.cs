...
{
    public static class LC_ContainmentUtility
    {
        ...
        public static bool SafelyContains(this Thing container, Thing thing)
        {
            ...
            Comp_LC_EntityContainer compEntityHolder = container.TryGetComp<Comp_LC_EntityContainer>();
        ...
        public static float GetEscapeIntervalMultiplier(Thing entity, Comp_LC_EntityContainer entityHolder)
        {
            return EscapeIntervalMultiplierFromContainmentStrengthCurve.
                Evaluate(entityHolder.ContainmentStrength - entity.GetStatValue(StatDefOf.MinimumContainmentStrength));
        }

        public static float GetStudyKnowledgeAmountMultiplier(Thing entity, Comp_LC_EntityContainer entityHolder)
        {
            return StudyKnowledgeMultiplierFromContainmentStrengthCurve.
                Evaluate(entityHolder.ContainmentStrength - entity.GetStatValue(StatDefOf.MinimumContainmentStrength));
        }
        ...
        public static bool IsLinearBuildingBlocked(ThingDef def, IntVec3 center, Rot4 rot, Map map, Thing thingToIgnore = null)
        {
            ...
            Verse.Building edifice = GetLinearBuildingFrontCell(def, center, rot).GetEdifice(map);
            if (edifice != null)
            {
        ...
        {
            Comp_LC_Studiable compStudiable = thing.TryGetComp<Comp_LC_Studiable>();
            if (compStudiable != null && thing is Pawn)
            {
                return compStudiable.RequiresHoldingPlatform;
            }

            if (thing.TryGetComp<Comp_LC_ContainingUnitTarget>()?.Props.heldPawnKind != null)
            {
                return true;
            }
        ...
        {
            if (thing.TryGetComp(out Comp_LC_ContainingUnitTarget comp) && comp.Props.getsColdContainmentBonus)
            {
        ...
        public static float InitiateEscapeMtbDays(Pawn pawn, StringBuilder sb = null)
        {
            ...
            Comp_LC_ContainingUnitTarget comp = pawn.GetComp<Comp_LC_ContainingUnitTarget>();
            ...
            float escapeIntervalMultiplier = GetEscapeIntervalMultiplier(pawn, comp.HeldPlatform.GetComp<Comp_LC_EntityContainer>());
