...
namespace LCAnomalyLibrary.Comp
{
    public class CompExtractRitualSpot : ThingComp
    {
        ...
        {
            foreach (Verse.Gizmo item in base.CompGetGizmosExtra())
            {
                yield return item;
            }
            ...
            {
                yield return ExtractRitualGizmo.CancelGizmo(psychicRitual);
            }
            else
            {
                foreach (Verse.Gizmo gizmo in ExtractRitualGizmo.GetGizmos(parent))
                {
                    if (Obstructed)
                    {
                        gizmo.Disabled = true;
