...
{
    public class Comp_LC_Studiable : ThingComp
    {
        ...
        public CompProperties_LC_Studiable Props => (CompProperties_LC_Studiable)props;
        ...
        public bool CurrentlyStudiable()
        {
            ...
            if (parent is Pawn pawn)
            {
                ...
                {
                    Comp_LC_ContainingUnitTarget compHoldingPlatformTarget = pawn.TryGetComp<Comp_LC_ContainingUnitTarget>();
        ...
        public bool EverStudiableCached(out string reason)
        {
            ...
            if (parent is Pawn pawn)
            {
                if (RequiresHoldingPlatform && !(parent.ParentHolder is Building.Building_LC_ContainingUnit))
                {
        ...
        private float AdjustedAnomalyKnowledge(StringBuilder sb = null)
        {
            ...
            bool flag = false;
            if (parent.TryGetComp<Comp_LC_ContainingUnitTarget>(out var comp) && comp.CurrentlyHeldOnPlatform)
            {
        ...
        {
            Comp_LC_Studiable compStudiable;
            if ((compStudiable = thing.TryGetComp<Comp_LC_Studiable>()) != null && compStudiable.DisplayStudyInfoOnInspectPane && compStudiable.EverStudiable())
            {
